...
function ProcessCard({ step }: { step: (typeof steps)[number] }) {
  return (
    <article className="group relative flex h-full flex-col rounded-[1.5rem] border border-white/75 bg-white/82 p-5 shadow-[0_18px_44px_-28px_rgba(15,23,42,0.22)] backdrop-blur-xl sm:rounded-[1.75rem] sm:p-6">
      ...
      <div className="mt-3 flex flex-1 flex-col">
        <div className={`mb-4 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br ${step.gradient} text-white shadow-lg sm:mb-5 sm:h-12 sm:w-12`}>
          <step.icon className="size-5" />
        </div>

        <h3 className="text-base font-black tracking-tight text-foreground sm:text-lg">{step.title}</h3>
        <p className="mt-3 text-sm font-medium leading-relaxed text-muted-foreground">{step.description}</p>

        <div className="mt-4 rounded-xl border border-white/70 bg-white/75 p-3 text-xs font-semibold text-foreground/80 shadow-sm sm:mt-5">
          {step.detail}
        </div>

        <div className="mt-4 inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground sm:mt-5 sm:text-xs">
